...
export function DateField({
  ...
  className,
  ...props
}: Omit<React.ComponentPropsWithoutRef<'input'>, 'id' | 'type'> & { label: string }) {
  let id = useId()

  return (
    <div className={className}>
      {label && <Label id={id}>{label}</Label>}
      <input id={id} type="date" {...props} className={formClasses} />
    </div>
  )
}
...
export function NumberField({
  ...
  return (
    <div className={className}>
      {label && <Label id={id}>{label}</Label>}
      <input id={id} type="number" {...props} className={formClasses} />
    </div>
  )
}
